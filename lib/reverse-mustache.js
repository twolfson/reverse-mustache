...
function parseTemplate(template, partials) {
  // Interpret the template
  var parsedTokens = mustache.parse(template);

  // Walk the tokens and reverse all partials
  var inlinedTokens = inlinePartials(parsedTokens, partials || {});
  return inlinedTokens;
}

// Define function to take care of any precomputed logic
function inlinePartials(tokens, partials) {
  ...
  for (; i < len; i++) {
    ...
    switch (type) {
      ...
      case '^': // Inverted if
        ...
        inlinePartials(subtokens, partials);
        break;
      case '>': // Partial (already reversed)
        // https://github.com/janl/mustache.js/blob/0.8.1/mustache.js#L497-L501
        var val = (typeof partials === 'function') ? partials(token[1]) : token[1];
        if (val !== null && val !== undefined) {
          tokens.splice(i, 1, parseTemplate(val, partials)[0]);
        }
        break;
    }
  }
...
function reverseMustache(params) {
  ...
  // Interpret the template
  var parsedTokens = mustache.parse(params.template);

  // Walk the tokens and reverse all partials
  var inlinedTokens = inlinePartials(parsedTokens, params.partials || {});

  console.log(inlinedTokens);
  // Run our depth first traversal
  var context = new Context(params.content);
  var result = dfs(context, inlinedTokens);
