...
Context.prototype = {
  ...
  getToken: function (key) {
    var val = pathval.get(this.tokensByName, key);
    return val;
  },
      ...
      case '#': // If/loop
        ...
        if (subresultArr.length) {
          ...
          for (; i > 0; i -= 1) {
            ...
            var proposedContext = context.clone();
            // TODO: Re-enable this as a fallback *after* `resultContext` and the `tokensByName` is empty. We have disabled it since we cannot predict global context references.
