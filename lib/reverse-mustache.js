  ...
  for (; i < len; i++) {
    ...
    switch (type) {
      ...
      case '^': // Inverted if
        ...
        console.log('inlining', token);
        token[4] = inlinePartials(subtokens, partials);
        console.log('inlining', token);
        break;
      case '>': // Partial (already reversed)
        ...
        if (val !== null && val !== undefined) {
          var partialTokens = parseTemplate(val, partials);
...
function reverseMustache(params) {
  ...
  console.log(JSON.stringify(inlinedTokens, null, 2));
