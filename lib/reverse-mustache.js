  ...
  function dfs(content, tokens, parentContext) {
    ...
    switch (type) {
      case '#': // If/loop
        ...
        console.log('hai', context.parentContext);
        if (val !== undefined) {
        ...
        while (true) {
          ...
          var subresult = dfs(loopContext.remainingContent, subtokens, context);
        ...
        if (subresultArr.length) {
          ...
          if (subresultArr.length === 1) {
            ...
            // Explore the tokens within our conditional
            var result = dfs(proposedContext.remainingContent, tokens.slice(1), context);
        ...
        var result = dfs(context.remainingContent, tokens.slice(1), context);
