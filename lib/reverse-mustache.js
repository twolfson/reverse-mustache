      ...
      case '#': // If/loop
        ...
        if (subresultContextArr.length) {
          ...
          for (; i > 0; i -= 1) {
            ...
            for (; j > 0; j -= 1) {
              // TODO: If we are on the total length, use `subresultContextArr[i]` as it is pre-calculated
              var proposedContent = subresultContext.completedContent.slice(0, j);

              // Match each subtoken for the amount of times it occurred
              ...
              // TODO: Try out this `new Array` syntax
              // var proposedVal = new Array(i).map(function (subresultContext) {
              var proposedVal = subresultContextArr.slice(0, i).map(function (subresultContext) {
                // If the object is empty, default to `true`
                // TODO: This makes no sense. Wtf am I doing here?
                // TODO: Test `{{#hello}}{{#world}}{{moon}} sun{{/world}}{{/hello}}` on 'hi sun wat sun'. I want to verify objects can contain objects, not some weird manipulation of `tokensByName` for an earlier match
                // TODO: This will probably match 'hi sun wat'. Ugh, being greedy is so damn aggressive o_o
                var tokensByName = subresultContext.tokensByName;
              ...
              proposedContext.addStr(subresultContextArr[i - 1].completedContent);
