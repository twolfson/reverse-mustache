...
function reverseMustache(params) {
  ...
  function dfs(content, tokens) {
    ...
    // TODO; Replace for loop with DFS traversal. A loop makes no sense =/
    for (; i < len; i++) {
      var token = tokens[i];
      var type = token[0];
      switch (type) {
        case '#': // If/loop
          ...
          if (subresult) {
            ...
            var wouldBeRemainingContent = context.remainingContent.slice(0, subresult.completedContent);
            var result = dfs(wouldBeRemainingContent, tokens.slice(1));

            // If we matched, accept boolean
            // TODO: Somehow inherit tokens
            if (result) {
              context.setToken(token[1], true);
              context.addStr(subresult.completedContent);
            }
