...
module.exports = function parserVariable (context, token, tokens, dfs) {
  // If the variable is already defined and is not a boolean (meaning it is not fully formed)
  var type = token[0];
  var name = token[1];
  ...
  for (; i >= 0; i -= 1) {
    ...
    // If we had a value
    if (val !== undefined) {
      // If it does not match the truthy/falsy, continue
      if (!!val !== !!proposedTokenVal) {
        continue;
      // Otherwise, silently set the variable
      } else {
        proposedContext.setToken(name, proposedTokenVal, true);
      }
    // Otherwise, set the variable normally
    } else {
